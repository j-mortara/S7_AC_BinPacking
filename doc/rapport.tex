...
On met les objets dans la boîte moins remplie parmis les boites déjà ouvertes.
On ouvre une nouvelle boîte seulement si la boîte la moins remplie ne peut pas contenir l'objet.
Si plusieurs boites ont une capacité égale on met l'objet dans la boîte ouverte en premier.
Complexité en $O(n^2)$.
\subsection{Implémentation de l'algorithme}
\begin{lstlisting}[language=Python, frame=single]
